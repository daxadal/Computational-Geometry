...
class MLP(object):
    ...
    def get_activations_and_units(self, x, parameters=None):
        ...
        parameters (real, vector): input value. If it's None, the function will
                                   ...
                                   to get activations and units. If not, it will
                                   use parameters as lists of weights and biases
        """
        # ->
    ...
    def train(self, x_data, t_data,
        ...
        def adadelta_update():
            ...
            # ->
            self.gradsquare_w_averages[k] *= gamma
            self.gradsquare_w_averages[k] += (1 - gamma) * self.grad_w_list[k]**2 #update E[g²]
            self.gradsquare_b_averages[k] *= gamma
            self.gradsquare_b_averages[k] += (1 - gamma) * self.grad_b_list[k]**2 #update E[g²]

            delta_w = np.sqrt(self.delta_w2[k] + epsilon)
            delta_w /= np.sqrt(self.gradsquare_w_averages[k] + epsilon) #get RMS[g]
            delta_w *= self.grad_w_list[k] #get Δθ
            self.weights_list[k] -= delta_w #update weights

            delta_b = np.sqrt(self.delta_b2[k] + epsilon)
            delta_b /= np.sqrt(self.gradsquare_b_averages[k] + epsilon) #get RMS[g]
            delta_b *= self.grad_b_list[k] #get Δθt
            self.biases_list[k] -= delta_b #update biases

            self.delta_w2[k] *= gamma
            self.delta_w2[k] += (1 - gamma) * delta_w**2 #update E[Δθ2]
            self.delta_b2[k] *= gamma
            self.delta_b2[k] += (1 - gamma) * delta_b**2 #update E[Δθ2]
            # <-
